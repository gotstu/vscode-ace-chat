...
// SPEX Search API: Expose search results for LLM analysis
export async function fetchSpexSearchResults(query: string, page = 1, limit = 10): Promise<SpexSearchResponse> {
    const url = `https://spex.se.com/api/version/1/search?q=${encodeURIComponent(query)}&page=${page}&limit=${limit}`;
    const response = await fetch(url);
    if (!response.ok) {
    ...
    return data as SpexSearchResponse;
}
